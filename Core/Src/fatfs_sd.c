...
uint16_t Get_YYMMDD(void);   // ½ÇÁ¦ ¹İÈ¯ Å¸ÀÔ¿¡ ¸Â°Ô ¼öÁ¤
...
typedef struct {
  uint8_t startFlag[2]; // ">>"
  char hhmm[4];         // '0', '9', '0', '0' ÇüÅÂÀÇ ¾Æ½ºÅ° ¹®ÀÚ¿­ (4byte)
  uint16_t len;         // µ¥ÀÌÅÍ ±æÀÌ (2byte)
} RecordHeader;
...
void SD_Write_Record(char* dirName, uint32_t yymmdd, uint16_t hhmm, const uint8_t* data, uint16_t len)
{
  ...
  // 2. ¼ıÀÚ hhmmÀ» 4ÀÚ¸® ¾Æ½ºÅ° ¹®ÀÚ¿­·Î º¯È¯ (¿¹: 900 -> "0900")
  // snprintf¸¦ »ç¿ëÇÏ¸é null ¹®ÀÚ°¡ µÚ¿¡ ºÙÀ¸¹Ç·Î 4¹ÙÀÌÆ®¸¸ ÁöÁ¤ÇÏ¿© µ¤¾î¾¹´Ï´Ù.
  char hhmmStr[5];
...
uint8_t SD_Read_Step_TxMsg(const char* dirName, uint32_t YYMMDD)
{
  ...
  if (hdr.startFlag[0] != '>' || hdr.startFlag[1] != '>')
  {
    ...
    const uint32_t MAX_SCAN = 4096;   // ÃÖ´ë ½ºÄµ ¹ÙÀÌÆ® Á¦ÇÑ
  ...
  // ¡Ú [ÇÙ½É º¯°æ] ¾Æ½ºÅ° ¹®ÀÚ ¹è¿­(4¹ÙÀÌÆ®) -> uint16_t ¼ıÀÚ(hhmm) º¯È¯
...
void SD_Delete_Record(const char* dirName, uint32_t YYMMDD, uint16_t hhmm)
{
    ...
    while(1)
    {
        ...
        // ÇÏÇÑ¼±(2)°ú »óÇÑ¼±(1300) µ¿½Ã °Ë»ç (CRC ¾ğ´õÇÃ·Î¿ì ¹× ¹öÆÛ ¿À¹öÇÃ·Î¿ì ¹æÁö)
        ...
        // ¡Ú [ÇÙ½É º¯°æ] ¾Æ½ºÅ° ¹®ÀÚ ¹è¿­(4¹ÙÀÌÆ®) -> uint16_t ¼ıÀÚ º¯È¯
        uint16_t parsed_hhmm = (hdr.hhmm[0] - '0') * 1000 +
                               (hdr.hhmm[1] - '0') * 100 +
                               (hdr.hhmm[2] - '0') * 10 +
                               (hdr.hhmm[3] - '0');
        ...
        if(parsed_hhmm == hhmm)
        {
            // Å¸°ÙÀ» Ã£À½! (´ÙÀ½ ·¹ÄÚµå À§Ä¡ = ÇöÀç Çì´õ À§Ä¡ + Çì´õ Å©±â + µ¥ÀÌÅÍ ±æÀÌ)
    ...
    if(found)
    {
        ...
        while(1)
        {
            ...
            // µ¤¾î¾²±â µµÁß ¿¡·¯(ºÎºĞ ¾²±â) ¹ß»ı ½Ã Áï½Ã Áß´Ü
...
uint8_t SD_GetLastHHMM(const char* dirName, uint32_t YYMMDD, uint16_t* outHHMM)
{
    ...
    while(1)
    {
        ...
        // ¡Ú [ÇÙ½É º¯°æ] 4¹ÙÀÌÆ® ¾Æ½ºÅ° ¹®ÀÚ¿­ -> uint16_t Á¤¼ö º¯È¯
        uint16_t parsed_hhmm = (hdr.hhmm[0] - '0') * 1000 +
                               (hdr.hhmm[1] - '0') * 100 +
                               (hdr.hhmm[2] - '0') * 10 +
                               (hdr.hhmm[3] - '0');

        // Á¤»ó Çì´õ È®ÀÎ ¿Ï·á -> ÃÖ½Å hhmm ÈÄº¸ °»½Å
        lastHHMM = parsed_hhmm;
        found = 1;

        // 3. ½ÇÁ¦ µ¥ÀÌÅÍ ¹ÙÀÌÆ®¸¸Å­ °Ç³Ê¶Ù±â (f_tell ¿ÀÇÁ¼Â ÀÌµ¿)
        if(f_lseek(&fil, f_tell(&fil) + hdr.len) != FR_OK)
            break;

        // ÆÄÀÏ ³¡À» ³Ñ¾î SeekµÇ¾ú´ÂÁö °Ë»ç (¹İÂÊÂ¥¸®/ºÒ¿ÏÀü ·¹ÄÚµå ¹æ¾î)
